...
public abstract class DataSelectionConfigurationBase : EdgeConfigurationBase, IDataSelectionConfiguration
{
    public bool Selected { get; set; } = true;

    [MinLength(OneCharacter)]
    [MaxLength(MaximumStreamIdLengthNoPrefix + MaximumStreamIdPrefixLength)]
    public string Name { get; set; }

    [Id]
    [MinLength(OneCharacter)]
    [MaxLength(MaximumStreamIdLengthNoPrefix)]
    public string StreamId { get; set; }

    [MinLength(OneCharacter)]
    [ForeignFacetKey(DataFiltersFacetName)]
    public string DataFilterId { get; set; }

    [JsonIgnore]
    public IDataFilterCache DataFilterCache { get; set; }

    public override IEnumerable<ValidationResult> Validate()
    {
        if (!string.IsNullOrWhiteSpace(StreamId))
        {
            if (StreamId.ToOmfIdentifier().Length > MaximumStreamIdLengthNoPrefix)
            {
        ...
        {
            if (Name.Length > MaximumStreamIdLengthNoPrefix + MaximumStreamIdPrefixLength)
            {
                var message = string.Format(
                    CultureInfo.InvariantCulture,
                    MaximumStringLengthExceededError,
                    nameof(Name),
                    MaximumStreamIdLengthNoPrefix + MaximumStreamIdPrefixLength);
