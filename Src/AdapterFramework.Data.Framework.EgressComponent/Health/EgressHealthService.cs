...
public class EgressHealthService : HealthServiceBase
{
    public EgressHealthService(
        IHealthMessageProcessor healthMessageProcessor,
        ILogger logger,
        IApplicationManifest applicationManifest,
        string componentId)
        : base(healthMessageProcessor, logger, componentId, EdgeSystemConstants.OmfEgressComponentType)
    {
        ThrowHelper.ThrowIfArgumentNull(applicationManifest, nameof(applicationManifest));

        var serviceNode = AdapterBaseHierarchyCreator.GetServiceNode(
            applicationManifest.HealthPrefix,
            applicationManifest.MachineName,
            applicationManifest.ServiceName);

        HealthMessageCreator = new EgressHealthOmfMessageCreator(
            applicationManifest,
            componentId,
            serviceNode);
    }
